...
// FIX: Import PanInfo to correctly type drag event handlers.
import {
  motion,
  useMotionValue,
  useSpring,
...
export const DraggableCardBody = ({
  ...
  // FIX: Replaced PanInfo with `any` as it was not found during module resolution.
  // Replaced `any` with the imported `PanInfo` type for better type safety.
  ...
  return (
    <MotionDiv
      ...
      onDrag={onDrag}
      // FIX: Removed explicit `any` types to allow for TypeScript inference from props.
    ...
    >
      ...
      <motion.div
        style={{
          ...
          // FIX: Cast `v` to number to resolve arithmetic operation type error.
          left: useTransform(mouseX, (v) => `${50 + (v as number) * 0.1}%`),
          top: useTransform(mouseY, (v) => `${50 + (v as number) * 0.1}%`),
          width: "200%",
          height: "200%",
        }}
